...
from scraper.img.image_tools import image_split, find_blocked_chapters
...
def get_device() -> Device:
    def get_device_from_client(client: AdbClient) -> Device:
        devices: list[Device] = client.devices()  # pyright:ignore[reportUnknownMemberType, reportUnknownVariableType]
        if not devices:
            devices = client.devices()  # pyright:ignore[reportUnknownMemberType, reportUnknownVariableType]
        if not devices:
...
def names_on_device(device: Device):
    res: str | None = device.shell(f"find {IMAGE_DIR} -type d")  # pyright:ignore[reportUnknownMemberType, reportUnknownVariableType]
    if res is None:
