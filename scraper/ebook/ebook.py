...
@app.command(
    help=(
        "Register a single string representing one or multiple lines "
        "that should be removed in every final epub. "
        "This only works if the text exactly matches line boundaries in the final epub"
    )
)
def epub_remove_lines(
    lines: Annotated[
        ...
        typer.Argument(
            help=(
                "String representing one or multiple lines (separated '\n')"
                " that should be removed in every final epub"
            )
        ),
    ],
