...
def download(
    ...
    out_directory_prefix: Annotated[
        Path,
        typer.Argument(
            help="Prefix used to place the files in. Images are placed as <out_directory_prefix>/<name>/downloaded images (single images) and out_directory_prefix/name/name for the batched images."
        ),
        ...
        with Progress(transient=True) as progress_bar:
            ...
            for it_name, data in progress.progress_by_name.items():
                if data.stopped:
                    ...
                    if chapter_targets:
                        print(f"targets: {chapter_targets}")
...
def download_targets(
    ...
    if batches:
        ...
        for name, paths in image_paths.items():
            out_directory = os.path.join(current_dir, name, name)
